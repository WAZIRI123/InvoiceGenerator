        ...
        <table class="w-full whitespace-no-wrap mt-4 shadow-2xl text-xs" wire:loading.class.delay="opacity-50">
            ...
            @foreach($results as $result)
                <tr class="hover:bg-blue-300 {{ ($loop->even ) ? "bg-blue-100" : ""}}">
                    ...
                    <td class="px-3 py-2" >
                        <!-- change {{ $result->id}} to { user: {{ $result->id}} } -->
                        <button type="submit" wire:click="$dispatchTo('wert-child', 'showEditForm', { user: {{ $result->id}} });" class="text-green-500">
                            <x-tall-crud-icon-edit />
                        </button>
                        <button type="submit" wire:click="$dispatchTo('wert-child', 'showDeleteForm', { user: {{ $result->id}} });" class="text-red-500">
