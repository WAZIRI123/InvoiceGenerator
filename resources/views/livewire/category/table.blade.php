...
@livewire('livewire-toast')
    <div class="flex justify-between">
        ...
        <button type="submit" wire:click="$dispatchTo('category.create', 'showCreateForm')" class="text-blue-500">
        ...
        <table class="w-full whitespace-no-wrap mt-4 shadow-2xl text-xs" wire:loading.class.delay="opacity-50">
            ...
            @foreach($results as $result)
                <tr class="hover:bg-blue-300 {{ ($loop->even ) ? "bg-blue-100" : ""}}">
                    ...
                    <td class="px-3 py-2" >
                        <button type="submit" wire:click="$dispatchTo('category.create', 'showEditForm', { category: {{ $result->id}} })" class="text-green-500">
                            <x-custom-components.icon-edit />
                        </button>
                        <button type="submit" wire:click="$dispatchTo('category.create', 'showDeleteForm', { category: {{ $result->id}} })" class="text-red-500">
